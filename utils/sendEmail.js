...
export const sendContactEmails = async (details) => {
  ...
  // --- 1. Admin Notification (Your Team)(contact us page Lead) ---
  const adminMailOptions = {
    from: `"Renny Strips Website" <${process.env.SMTP_USER}>`,
    to: process.env.ENQUIRY_EMAIL, // Your office emails
    subject: `New Lead: ${details.enquiryType} - ${details.fullName}`,
    html: `
      <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; line-height: 1.6; color: #333333; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
        <!-- Header -->
        <div style="background: #292c44; padding: 24px 32px; color: #ffffff; text-align: center;">
          <h2 style="margin: 0; font-size: 22px; font-weight: 600; letter-spacing: 0.5px;">New Website Enquiry</h2>
          <p style="margin: 8px 0 0 0; font-size: 14px; color: #b0b3c6;">A potential lead has submitted an enquiry through the website.</p>
        </div>
        
        <!-- Content Body -->
        <div style="padding: 32px; background: #ffffff;">
          <h3 style="margin-top: 0; margin-bottom: 20px; font-size: 18px; color: #292c44; border-bottom: 2px solid #f0f0f0; padding-bottom: 10px;">Lead Details</h3>
          
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
            <tr>
              <td style="padding: 10px 0; font-size: 14px; color: #666666; width: 30%; font-weight: bold; vertical-align: top;">Name</td>
              <td style="padding: 10px 0; font-size: 15px; color: #222222; font-weight: 500; vertical-align: top;">${details.fullName}</td>
            </tr>
            <tr>
              <td style="padding: 10px 0; font-size: 14px; color: #666666; font-weight: bold; vertical-align: top;">Email</td>
              <td style="padding: 10px 0; font-size: 15px; color: #222222; vertical-align: top;"><a href="mailto:${details.email}" style="color: #1a73e8; text-decoration: none;">${details.email}</a></td>
            </tr>
            <tr>
              <td style="padding: 10px 0; font-size: 14px; color: #666666; font-weight: bold; vertical-align: top;">Phone</td>
              <td style="padding: 10px 0; font-size: 15px; color: #222222; vertical-align: top;"><a href="tel:${details.phoneNumber}" style="color: #1a73e8; text-decoration: none;">${details.phoneNumber}</a></td>
            </tr>
            <tr>
              <td style="padding: 10px 0; font-size: 14px; color: #666666; font-weight: bold; vertical-align: top;">Enquiry Type</td>
              <td style="padding: 10px 0; font-size: 15px; vertical-align: top;">
                <span style="background-color: #e8f0fe; color: #1a73e8; padding: 4px 10px; border-radius: 12px; font-size: 13px; font-weight: 600; display: inline-block;">
                  ${details.enquiryType}
                </span>
              </td>
            </tr>
          ...
          <h3 style="margin-top: 0; margin-bottom: 12px; font-size: 16px; color: #292c44; font-weight: 600;">Message:</h3>
          <div style="background-color: #f8f9fa; border-left: 4px solid #292c44; padding: 18px; border-radius: 4px; font-size: 14px; color: #444444; line-height: 1.6; white-space: pre-wrap;">
            ${details.message}
          </div>
        </div>

        <!-- Footer -->
        <div style="background: #f4f5f7; padding: 20px 32px; text-align: center; border-top: 1px solid #e0e0e0;">
          <p style="margin: 0; font-size: 12px; color: #888888;">This email was automatically generated by the Renny Strips Website enquiry form.</p>
          <p style="margin: 4px 0 0 0; font-size: 11px; color: #aaaaaa;">&copy; ${new Date().getFullYear()} Renny Strips Limited. All rights reserved.</p>
        </div>
      </div>
    `,
  };
